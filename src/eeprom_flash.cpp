...
void eeprom_flash::read_all_data()
{
    ...
    ec11.int_close();
    EEPROM.begin(eeprom_size);  // 初始化eeprom大小
    ...
    else
    {
        ...
        pwm.constant_temp_time = ((uint16_t)(EEPROM.read(temp_mode1_time_high_add) << 8)) | EEPROM.read(temp_mode1_time_low_add);
        ...
        {
            adc.hotbed_max_temp = 255;
            EEPROM.write(hotbed_max_temp_low_add, adc.hotbed_max_temp & 0xff);  // 得到低八位
            EEPROM.write(hotbed_max_temp_high_add, adc.hotbed_max_temp >> 8);  //得到高八位
        }
        // ----------------------------------------------------------------
...
void eeprom_flash::write_task()
{
    if (write_flg == 2)
    {
        ...
        EEPROM.write(temp_mode1_time_low_add, pwm.constant_temp_time & 0xff);
        EEPROM.write(temp_mode1_time_high_add, pwm.constant_temp_time >> 8);
