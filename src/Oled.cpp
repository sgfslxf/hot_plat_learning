...
void write_oled(unsigned char dat, unsigned char mode) // 根据不同平台更改IIC或SPI发送函数
{
	...
	if (mode)							// 判断是指令还是数据
	{
		Wire.write(0x40); // 指定写数据内容
	}
	else
	{
		Wire.write(0x00); // 指定写指令内容
	}
	Wire.write(dat);		// 写指令或数据内容
...
void Oled::begin() // IIC/SPI初始化函数，根据平台自行修改
{
#if WRITE_MODE
	{
		...
		Wire.setClockStretchLimit(1000000); // 设置时钟拉伸限值
		Wire.setClock(1000000);				// 设置时钟
		init();
	}
#else
	{
...
void Oled::init()
{
	...
	{
		write_oled(0x40, OLED_CMD); // 从RAM中哪一行起读取显示内容 设置显示起始线(40h~7Fh)
		write_oled(0xB0, OLED_CMD); // 该命令在页寻址模式下将 GDDRAM 中的页起始地址定位为 0 到 7
		write_oled(0xC8, OLED_CMD); // 设置输出行扫描顺序，不颠倒
		write_oled(0xa1, OLED_CMD); // 段重定向设置
		write_oled(0xa6, OLED_CMD); // 颜色显示方式正常显示
		write_oled(0xa8, OLED_CMD); // 设置驱动路数
		write_oled(0x1f, OLED_CMD); // 设置起始列地址的高位

		write_oled(0xd3, OLED_CMD); // 设置显示偏移

		write_oled(0x00, OLED_CMD); // 设置起始列地址的低位
		write_oled(0xd5, OLED_CMD); // 设置时钟分频因子,震荡频率
		write_oled(0xf0, OLED_CMD); // 频率设置成最高407 kHz，分频系数为1
		write_oled(0xd9, OLED_CMD); // 设置预充电周期
		write_oled(0x22, OLED_CMD); // 设置页地址区域
		write_oled(0xda, OLED_CMD); // 设置COM引脚硬件配置
		write_oled(0x02, OLED_CMD); // 页寻址
		write_oled(0xdb, OLED_CMD); // 设置COM的反压
		write_oled(0x49, OLED_CMD);
		write_oled(0x8d, OLED_CMD); // 电荷泵设置
		write_oled(0x14, OLED_CMD); // 使能电荷泵
	}
#endif
...
void Oled::choose_clr(unsigned char x, unsigned char y, unsigned char width, unsigned char height)
{
	...
	if(y + height > PAGE_SIZE || x + width > WIDTH)
	{
		Serial.print("out scope!\n");
		return;
	}
...
/// @brief 显示一个数字，当x,y为0，且为右对齐时，相当于不显示
/// @param x 小图标左上角的0-128范围位置xx
/// @param y 小图标左上角的0-32范围位置yy
/// @param num 要显示的数字
/// @param len 要显示的数字的长度
/// @param Size 数字显示大小 有8和16两种
/// @param align 对齐方式，有 UP,DOWN,LEFT,RIGHT四种
...
/// @brief 设定显示函数边界, 超出部分将在显示字符串\中文\位图 显示函数中不显示,x xx:0-128,y yy:0-4
/// @param x xy为左上角
/// @param y xy为左上角
/// @param xx xx yy为右下角
/// @param yy xx yy为右下角
void Oled::xy_set(unsigned char x, unsigned char y, unsigned char xx, unsigned char yy)
{
	Oled_roll_x[0] = x;
	Oled_roll_x[1] = xx;
	if (yy > PAGE_SIZE)  // y一直为0，也就是yy大于4就等于4
		Oled_roll_x[3] = PAGE_SIZE;
	else
		Oled_roll_x[3] = yy;
}
